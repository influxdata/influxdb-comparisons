...
func (d *InfluxDashboardQueueBytes) Dispatch(i int) bulkQuerygen.Query {
	...
	query = fmt.Sprintf("SELECT mean(\"temp_files\") FROM postgresl WHERE cluster_id = '%s' and time >= '%s' and time < '%s' group by time(1m), hostname fill(0)", d.GetRandomClusterId(), interval.StartString(), interval.EndString())
