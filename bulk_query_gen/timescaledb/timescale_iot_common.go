...
func NewTimescaleIotCommon(dbConfig bulkQuerygen.DatabaseConfig, start, end time.Time) bulkQuerygen.QueryGenerator {
	if !start.Before(end) {
...
func (d *TimescaleIot) Dispatch(i, scaleVar int) bulkQuerygen.Query {
	q := NewSQLQuery() // from pool
	bulkQuerygen.IotDispatchAll(d, i, q, scaleVar)
	return q
}

func (d *TimescaleIot) AverageTemperatureDayByHourOneHome(q bulkQuerygen.Query, scaleVar int) {
	d.averageTemperatureDayByHourNHomes(q.(*SQLQuery), scaleVar, 1, time.Hour*6)
}
...
func (d *TimescaleIot) averageTemperatureDayByHourNHomes(qi bulkQuerygen.Query, scaleVar, nHomes int, timeRange time.Duration) {
